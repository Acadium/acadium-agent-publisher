 ...
 *
 *   agent-publisher/list-terms         read   list categories or tags
 *   agent-publisher/get-post           read   read one post the user can edit
 *   agent-publisher/create-draft-post  write  create a post as a draft
 *   agent-publisher/update-draft-post  write  change a post that is still a draft
 *   agent-publisher/upload-media       write  add an image to the Media Library
 *
 * The MCP Adapter plugin exposes them to MCP clients through its
 * discover / get-info / execute tools. Each ability runs as the WordPress user
 * the client authenticated as (Application Password), and WordPress'
 * normal capability checks and HTML filtering apply.
 *
 * Also adds the role "AI Agent (drafts only)" (`agent_publisher_agent`). Give the agent's
 * WordPress user this role: Application Passwords work for the whole REST API,
 * not just these abilities, so the role itself must be unable to publish or to
...
final class Agent_Publisher {
	...
	/**
	 * Drafts only: create/edit/delete its own unpublished posts and upload
	 * media. No publish_posts, edit_published_posts, delete_published_posts,
	 * edit_others_posts or unfiltered_html.
	 */
	...
	public static function init() {
		add_action( 'init', function () {
			if ( (int) get_option( 'agent_publisher_role_version' ) !== self::ROLE_VERSION ) {
				self::add_role();
			}
		} );
...
Agent_Publisher::init();
register_activation_hook( __FILE__, array( 'Agent_Publisher', 'add_role' ) );
